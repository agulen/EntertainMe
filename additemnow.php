 ...
 if (isset($_POST['additem']) && $_POST['additem'] == 'Submit') {
    ...
    if (!isset($_POST['title']) || empty($_POST['title'])) {
      ...
      $stmt->execute(array(':title' => $_POST['title'],
                           ...
                           ':type' => $_POST['dropdown']));
      $insertsql = "INSERT INTO now (entertainment_id)
      					SELECT MAX(id) FROM entertainment";
      $stmt2 = $conn->query($insertsql);

      $updatesql = $conn->prepare("UPDATE now SET username=:user WHERE entertainment_id=MAX(entertainment_id)");
  ...
  //NOW and LATER are made up of foreign keys....preventing updates and inserts. HOW TO FIX???
